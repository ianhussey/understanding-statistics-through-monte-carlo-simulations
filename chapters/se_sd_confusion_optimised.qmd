...
# wrapper function: generates data and analyses it in a single call
# this avoids storing intermediate generated data in a list-column,
# reducing memory usage and allowing a single future_pmap call per simulation
simulate_once <- function(n_per_condition,
                          mean_control,
                          mean_intervention,
                          sd,
                          ...) {

  # capture all input parameters so they can be returned alongside results
  params <- tibble(n_per_condition = n_per_condition,
  ...
  data <- bind_rows(data_control, data_intervention)
  ...
  # return parameters and results together
  bind_cols(params,
            tibble(cohens_d_correct = cohens_d_correct,
...
experiment_parameters_1 <- expand_grid(
  ...
  true_effect = c(0, 0.2, 0.5, 0.8, 1, 1.5, 2, 3, 4, 5),
  sd = 1,
  iteration = 1:1000
) |>
...
# run simulation
simulation_1 <- future_pmap_dfr(.l = experiment_parameters_1,
                                .f = simulate_once,
                                .progress = TRUE,
...
# run simulation
simulation_2 <- future_pmap_dfr(.l = experiment_parameters_2,
                                .f = simulate_once,
                                .progress = TRUE,
                                .options = furrr_options(seed = TRUE))
...
experiment_parameters_3 <- expand_grid(
  n_per_condition = seq(from = 5, to = 60, by = 5),
  mu = c(0, 0.2, 0.5, 0.8, 1, 1.5, 2, 3, 4, 5),
  tau = c(0, 0.1, 0.2, 0.4),
  iteration = 1:1000
) |>
  mutate(
...
# run simulation
simulation_3 <- future_pmap_dfr(.l = experiment_parameters_3,
                                .f = simulate_once,
                                .progress = TRUE,
                                .options = furrr_options(seed = TRUE))
